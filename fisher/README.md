...
Fisher **will _NOT_** work on private servers!
This is due to the fact that private servers do not properly implement the fishing system on the server side.
There is nothing that can be done to fisher to fix this. It's a problem with the server and needs to be fixed there.
...
Since `rotten meat` can be used for bait and can also be fished up as an item, different names are used in fisher.
If you want to add/remove it as bait, use `rotten meat` as the name.
If you want to add/remove it as something to catch, use `rotten meat catch` as the name.
